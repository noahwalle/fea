...
def test2024_2():
    ...
    load = -4e3
    f_x = load * np.cos(alpha_2) # has to be positive alpha
    f_y = load * np.sin(alpha_2) # has to be positive alpha
...
def assignment_1():
    ...
    a1 = 0
    a2 = np.degrees(np.arctan(1.2/(4/3)))
    a3 = 0
    a4 = np.degrees(np.arctan(0.8/(2/3)))
    a5 = np.degrees(np.arctan(1.2/(-2/3)))
    a6 = np.degrees(np.arctan(0.8/(-4/3)))
    a7 = np.degrees(np.arctan(0.4/(-4/3)))
    a8 = -180

    l1 = 2
    l2 = np.sqrt(1.2**2 + (4/3)**2)
    l3 = 2
    l4 = np.sqrt(0.8**2 + (2/3)**2)
    l5 = np.sqrt(1.2**2 + (2/3)**2)
    l6 = np.sqrt(0.8**2 + (4/3)**2)
    l7 = np.sqrt(0.4**2 + (4/3)**2)
    l8 = 4/3
    ...
    # structure1.A_matrices()
    # structure1.draw_structure_initial()
    # structure1.draw_structure_deflected(deflection_scale=5)
    # structure1.show_structure()
    ...
    structure2.draw_full_deflected_frame(deflection_scale=5, n_points=20)
